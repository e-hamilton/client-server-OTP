...
/*
**	"otp_enc_d.c"
** 	Usage:	enc_dec [open port]
**	otp_enc_d is a daemon for otp_enc clients. When a client connects to its
** 	port, otp_enc_d accepts a message and checks that the message code equals
**	"E". If the code is incorrect, it returns the message "FAIL" and goes back
...
**	it receives the key, it uses it to encrypt the first message and sends
**	the results to the client.
**	otp_enc_d can handle concurrent connections.
*/
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		char* completeMessage, *keyMessage, *encryptedMsg;
		...
		switch(pid) {
			case -1:	// fork() error
						fprintf(stderr, "Error: Couldn't fork new process.\n");
						break;
			case 0:		// This is the child process.
						...
						if (err == 0) {
							...
							if (err == 0) {
								...
								encryptedMsg = malloc(sizeof(char) * MAX_MESSAGE);
								memset(encryptedMsg, '\0', MAX_MESSAGE);
								encrypt(completeMessage, keyMessage, encryptedMsg, strlen(completeMessage));
								strcat(encryptedMsg, "$$");
								//printf("\n\nSERVER: Encrypted Message: <%s>\n\n", encryptedMsg);

								//charsRead = send(establishedConnectionFD, "SUCCESS", 7, 0);
								charsRead = send(establishedConnectionFD, encryptedMsg, strlen(encryptedMsg), 0);
								if (charsRead < strlen(encryptedMsg)) softError("SERVER: WARNING: Not all data written to socket!");
							}
							else {
